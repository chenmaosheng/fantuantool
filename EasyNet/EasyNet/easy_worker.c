			...
			case OPERATION_ACCEPT:
				{
					...
					{
						int32 rc = 0;
						int32 local_addr_len = 0, remote_addr_len = 0;
						getacceptexsockaddrs_(pAcceptor->context_.buffer_, 0, sizeof(SOCKADDR_IN), sizeof(SOCKADDR_IN), (struct sockaddr**)&pConnection->local_sockaddr_, &local_addr_len, 
							(struct sockaddr**)&pConnection->remote_sockaddr_, &remote_addr_len);
			...
			case OPERATION_CONNECT:
				{
					if (bResult)
					{
						int32 rc = setsockopt(pConnection->socket_, SOL_SOCKET, SO_UPDATE_CONNECT_CONTEXT, NULL, 0);
						_ASSERT(rc == 0);
						if (rc == 0)
						{
							if (pConnection->handler_.OnConnection((ConnID)pConnection))
