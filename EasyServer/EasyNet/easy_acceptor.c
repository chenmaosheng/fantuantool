...
EasyAcceptor* CreateAcceptor(PSOCKADDR_IN addr, EasyWorker* pWorker, EasyHandler* pHandler)
{
	int32 rc = 0;
	DWORD val = 0;
	...
	if (pAcceptor)
	{
		...
		if (!InterlockedCompareExchange(&pAcceptor->running_, 1, 0))
		{
			// start acceptor
...
void SetServerApp(EasyAcceptor* pAcceptor, void* pServer)
{
	pAcceptor->server_ = pServer;
}

void* GetServerApp(EasyAcceptor* pAcceptor)
{
	return pAcceptor->server_;
}
