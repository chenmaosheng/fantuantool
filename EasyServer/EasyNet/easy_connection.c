...
void DeleteConnection(EasyConnection* pConnection)
{
	closesocket(pConnection->socket_);
...
void AsyncDisconnect(EasyConnection* pConnection)
{
	...
	if (InterlockedCompareExchange(&pConnection->connected_, 0, 1))
	{
...
void SetClientApp(EasyConnection* pConnection, void* pClient)
{
	pConnection->client_ = pClient;
}

void* GetClientApp(EasyConnection* pConnection)
{
	return pConnection->client_;
}

BOOL IsConnected(EasyConnection* pConnection)
{
	return pConnection->connected_ ? TRUE : FALSE;
}
